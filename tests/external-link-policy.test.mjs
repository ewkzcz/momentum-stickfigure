...
async function fixture(t) {
  ...
  class Window extends EventEmitter {
    constructor() {
      ...
      Object.assign(this.webContents, { session: { webRequest: { onBeforeRequest() {}, onHeadersReceived() {} }, setPermissionRequestHandler() {} }, setWindowOpenHandler: handler => { this.open = handler } })
    }
...
for (const target of ['main', 'webview']) test(`外部链接${target}：仅HTTP(S)交给系统，拒绝本地与脚本协议`, async t => {
  ...
  assert.deepEqual(f.opened, ['https://github.com/ewkzcz/momentum-stickfigure', 'http://localhost:8080/docs', 'https://example.com/help'])
})

...
import { preflightPsdResources, assertPsdOutputBudget } from './psd-resource-budget.mjs'

// 1、原解析器的同步工作由主进程计时并终止，此处保留其默认解析及错误语义。
try {
  preflightPsdResources(workerData.fileBuffer, workerData.options?.parseImages)
  const result = await parsePSD(workerData.fileBuffer, workerData.options)
  assertPsdOutputBudget(result)

/**
 * 对话框画布交互：集中管理控制手柄、像素命中与对象变换操作。
 * 处理流程：
 * 1、接收父组件唯一的画布引用、尺寸与拖拽状态，以及绘制模块的图片和绘制入口。
 * 2、在本模块维护唯一的手柄状态，按原流程处理鼠标、滚轮和工具栏操作。
 * 3、返回父组件公开的交互入口及绘制模块延迟调用的选框、手柄函数。
 * 画布布局、嵌入模式、数据观察器与生命周期继续由父组件维护。
 */
import { ref } from 'vue'
import { clamp, calculateContainSize } from '../utils/helpers.js'

export function useDialogCanvasInteraction({
  props,
  emit,
  canvas,
  ctx,
  ...
  // 仅公开父组件原有入口与绘制所需回调，手柄状态和命中辅助函数保持模块私有。
  return {
    drawSelectionBox,
    getHandleMetrics,
    getTransformHandles,
